...
The package requires exact `wine=11.15-1`, matching the only runtime payload accepted by wrapper 0.1.4. Source installs can provision that same immutable Arch Linux Archive package privately when the exact system package is unavailable. Every Wine file and link is checked against the shipped runtime manifest. The wrapper privately deploys the audited DXVK D3D9 DLL, so it does not depend on `dxvk-bin`.
